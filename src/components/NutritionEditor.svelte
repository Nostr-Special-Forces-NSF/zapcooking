  ...
  {#if Object.keys($nutritionValues).length > 0}
	<ul class="flex flex-col gap-2 mb-4">
	  {#each Object.entries($nutritionValues) as [key, value]}
		<li class="flex input items-center" transition:slide|global={{ duration: 300 }}>
		  {#if editingKey === key}
			<!-- Edit mode -->
		  ...
		  {:else}
			<!-- Display mode -->
			<span class="grow flex items-center gap-1">
			  {#if nutritionTypes.find(nt => nt.key === key)?.emoji}
				<span>{nutritionTypes.find(nt => nt.key === key)?.emoji}</span>
			  {/if}
			  <span on:dblclick={() => startEditing(key)} class="cursor-pointer">
