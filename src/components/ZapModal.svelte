<script lang="ts">
  ...
  function subscribeToZapReceipt(pubkey: string, expectedInvoice: string) {
    const sub = $ndk.subscribe(
      { kinds: [9735], '#p': [pubkey], '#e': [event.id] },
      { closeOnEose: false }
    );
  ...
  $: isPaid = (pubkey: string) =>
    paymentStatuses.some((status) => status.pubkey === pubkey && status.paid);
</script>
  ...
  <div class="flex flex-col gap-3">
    {#if state == 'pending'}
      <div class="flex flex-col text-2xl">
        ...
        <span class="self-center"
          >{useQR ? 'Fetching Invoice(s)...' : 'Waiting for Payment...'}</span
        >
        {#if useQR}
    ...
    {:else if state == 'error'}
      <div class="flex flex-col items-center justify-center">
        <XIcon color="red" weight="bold" class="h-36 w-36" />
        <span class="ml-4 text-center text-2xl"
          >An Error Occurred. <br /> {error && error.toString()}</span
        >
      </div>
      ...
      {#if useQR == true}
        <div class="flex flex-col gap-3">
          <div class="flex gap-3 text-lg">
            <Avatar
              class="h-14 w-14 self-center rounded-full"
              ndk={$ndk}
              pubkey={paymentsToMakeQR[selected_qr - 1].recipientPubkey}
            />
            <div class="gap-1 self-center">
              Zapping <span class="font-semibold">{amount} sats</span> to <Name
                class="font-semibold"
                ndk={$ndk}
                pubkey={paymentsToMakeQR[selected_qr - 1].recipientPubkey}
              />
          ...
          {#if isPaid(paymentsToMakeQR[selected_qr - 1].recipientPubkey)}
            <div class="flex flex-col items-center justify-center">
              <Checkmark color="#90EE90" weight="fill" class="h-36 w-36" />
              <span class="ml-4 text-2xl">Payment Completed</span>
            </div>
          {:else}
